...
There are two versions of CHURP available: `0.2.2-slurm` and `0.2.3`. We will
load the latest version of the module. This is command **3** in the text file:
...
kilobase" value, which is the fragment counts divided by the length of the
...
sum of all per-feature TPM values will be equal across samples. This makes it
useful for comparing relative expression values both between samples and within
a sample.
